...
function handleClickColor(value: string, e: Event) {
  activeSelectColor(value, e.target as HTMLElement)
  // 选择默认色彩梯度
  if (value === '#262626') {
...
// 激活颜色排他
function activeSelectColor(value: string, target: HTMLElement) {
  Array.from((colorWrap.value as HTMLElement).children).forEach((row) => {
    Array.from(row.children).forEach((item) => {
      item.classList.remove('color-panel-item-active')
    })
  });
  target.classList.add('color-panel-item-active')
